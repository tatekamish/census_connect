...
def get_table(get_vars, pred_for, pred_in, year):
    ...
    #csvt_text = '''"Integer","String","String","String"'''
...
def get_geo(geo_level, year, state):
    ...
    if geo_level == "*":
        ...
        pass
    url_geo_level = geo_level    # "COUNTY", "STATE", etc. 
    url_year = year            # "####"
    url_state = state          #2-dig state code
    url_geo_lower = url_geo_level.lower() + f"{year[2:]}" #used in url_suffix (file name)
...
# TEST:
geo_url, url_suffix, shp_join_field  = get_geo('*', '2010', '42')
